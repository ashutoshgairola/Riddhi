...
export class SettingsController {
  ...
  updateUserPreferences = async (
    ...
    try {
      ...
      const preferences = await this.settingsService.updateUserPreferences(
        userId,
        updates
      );
      res.status(200).json(preferences);
  ...
  updateNotificationSetting = async (
    ...
    try {
      ...
      try {
        const setting = await this.settingsService.updateNotificationSetting(
          id,
          userId,
          updates
        );
  ...
  getAccountConnections = async (
    ...
    try {
      ...
      const connections = await this.settingsService.getAccountConnections(
        userId
      );
  ...
  connectAccount = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!connectionData.institutionId || !connectionData.credentials) {
        res
          .status(400)
          .json({ error: "Institution ID and credentials are required" });
        return;
      }

      try {
        const connection = await this.settingsService.connectAccount(
          userId,
          connectionData
        );
  ...
  refreshConnection = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      try {
        const connection = await this.settingsService.refreshConnection(
          id,
          userId
        );
  ...
  exportData = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (query.format !== "json" && query.format !== "csv") {
        res
          .status(400)
          .json({ error: "Invalid format. Supported formats: json, csv" });
        return;
      }

      try {
        const { filePath, contentType } = await this.settingsService.exportData(
          userId,
          query
        );
  ...
  importData = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!type || !["transactions", "budgets", "goals"].includes(type)) {
        res.status(400).json({
          error:
            "Invalid data type. Supported types: transactions, budgets, goals",
        });
        return;
      }
  ...
  clearData = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Validate request
      if (
        !Array.isArray(clearRequest.types) ||
        clearRequest.types.length === 0
      ) {
      ...
      if (
        !clearRequest.confirmation ||
        clearRequest.confirmation !== "DELETE MY DATA"
      ) {
        res.status(400).json({
