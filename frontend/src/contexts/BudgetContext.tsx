...
export const BudgetProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const getCurrentBudget = async (): Promise<Budget | null> => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch current budget"
      );
      return null;
    } finally {
      setIsLoading(false);
  ...
  const getBudgetHistory = async (): Promise<Budget[]> => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch budget history"
      );
      return [];
    } finally {
      setIsLoading(false);
  ...
  const deleteBudget = async (id: string): Promise<void> => {
    try {
      ...
      if (currentBudget && currentBudget.id === id) {
        ...
        const updatedHistory = budgetHistory.filter(
          (budget) => budget.id !== id
        );
        setBudgetHistory(updatedHistory);
      }
  ...
  const addBudgetCategory = async (
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to add budget category"
      );
  ...
  const deleteBudgetCategory = async (
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to delete budget category"
      );
